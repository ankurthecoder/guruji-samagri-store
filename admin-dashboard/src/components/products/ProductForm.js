...
const ProductForm = ({ product, onSave, onCancel }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (!formData.unit && unitData.units.length > 0) {
    ...
    const handleAddUnit = async () => {
        ...
        try {
            setUnitLoading(true);
            const response = await unitService.createUnit({ name: newUnit });
            if (response.success) {
                setUnits(prev => [...prev, response.unit].sort((a, b) => a.name.localeCompare(b.name)));
                setFormData(prev => ({ ...prev, unit: response.unit.name }));
                setNewUnit('');
            ...
            <Dialog open={unitDialogOpen} onClose={() => setUnitDialogOpen(false)}>
                ...
                <DialogContent>
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        Enter the name of the new unit (e.g., kg, gram, pack).
                    </Typography>
                    <TextField
                        autoFocus
                        fullWidth
                ...
                <DialogActions sx={{ p: 2, pt: 0 }}>
                    ...
                    <Button
                        ...
                        variant="contained"
                        disabled={unitLoading || !newUnit.trim()}
                        sx={{ bgcolor: COLORS.PRIMARY, '&:hover': { bgcolor: '#0A6B18' } }}
                    >
                        {unitLoading ? <CircularProgress size={24} color="inherit" /> : 'Add Unit'}
                    </Button>
                </DialogActions>
            </Dialog>
        </Box>
